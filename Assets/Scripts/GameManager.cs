...
public class GameManager : MonoBehaviour
{
    ...
    int overall_score = 0,curr_chance = 0,curr_throw = 1;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.name == "Ball1")
        {
            ...
            if (curr_frame == 3)
            {            
                //my_db.scores[my_db.cur] = overall_score;
                my_db.prev = overall_score;
                //my_db.cur = (my_db.cur + 1)%10;

                using (StreamWriter sw = File.AppendText(path)) {
                    sw.WriteLine(overall_score.ToString());
                }
